    ...
    try :
        ...
        for j in range(2, num+2):
            ...
            for i in content:
                ...
                data_evaluation_2 = i.find('div', 'css-0 eu87mqk15').text
                data_evaluation_ = re.sub('[^0-9]', '', data_evaluation_2)
        ...
        for pages in data_url :
            ...
            if price_1 :
                pay = content.find('section', 'css-16df71f e12i9j8n0').find_all('div', 'css-d0415h e12i9j8n1')
                data_STANDARD_ = re.sub('[^0-9]', '', pay[0].text)
                data_DELUXE_ = re.sub('[^0-9]', '', pay[1].text)
                data_PREMIUM_ = re.sub('[^0-9]', '', pay[2].text)

            elif price_2:
                pay = content.find('div', 'css-1it4gjn ea0crmh2').text
                data_STANDARD_ = re.sub('[^0-9]', '', pay)
                data_DELUXE_ =  '가격 없음'
                data_PREMIUM_ = '가격 없음'

            data_wishlist_2 = content.find('section', 'css-29iuxd e1stf3gr4').find('span', 'css-1oteowz eklkj753').text
            data_wishlist_ = re.sub('[^0-9]', '', data_wishlist_2)
            data_operations_2 = content.find('span', 'css-8ioq0m ec3naz84').text
            data_operations_ =  re.sub('[^0-9]', '', data_operations_2)

            data_middle_ = soup.find_all('div', 'css-70qvj9 e934qa11')[2].text
            try :
                data_sub_ = soup.find_all('div', 'css-70qvj9 e934qa11')[3].text
            except :
                data_sub_ = '-'

            data_middle.append(data_middle_)
            data_sub.append(data_sub_)
        ...
        print('An error occurred in', categorys_name)
